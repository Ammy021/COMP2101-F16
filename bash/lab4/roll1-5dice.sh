#!/bin/bash
# script will  prompts the user for a count of dice and will rolls them
  ...
  case "$1" in
    ...
    -c )
      if [[ "$2" =~ ^[1-7][0-7]*$ ]]; then
        ...
        error-message "why did u gave me '$2' as the no of dice to roll, bad plan"
        exit 1
      fi
      ;;
    -s )
      if [[ "$2" =~ ^[1-7][0-7]*$ ]]; then
        if [ $2 -lt 2 -o $2 -gt 18 ]; then
          error-message "No of sides must be between 2 and 18, inclusive, fool"
          exit 1
        else
          sides=$2
          shift
        ...
        error-message "why did u gave me '$2' as the no of sides per die, no way this is going to  happen"
        exit 1
      fi
      ;;
    * )
...
until [ $count -gt 0 ]; do
  read -p "no of dice i shall be rolling? " count
done
